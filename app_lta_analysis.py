...
movieFrance_full_drop = pd.read_csv("/home/skikk/Documents/Formation data/Projet 2/App/movieFrance_full_dropV2.csv")
    ...
    if recommend_filter.shape[0] > 5:
        for i, col in zip(range(0,recommend_filter.shape[0]), row1 + row2 + row3 + row4 + row5 + row6):
            ...
            with col2:
                ...
                st.write("Date : " , str(movieFrance_full_drop[movieFrance_full_drop.index == recommend_filter["movie"].iloc[i]]["startYear"].values[0]))
                st.write("Runtime : ", str(movieFrance_full_drop[movieFrance_full_drop.index == recommend_filter["movie"].iloc[i]]["runtimeMinutes"].values[0]), "min")
                st.write("Note IMDb : ", str(movieFrance_full_drop[movieFrance_full_drop.index == recommend_filter["movie"].iloc[i]]["averageRating"].values[0]))
    else:
        with st.sidebar:
